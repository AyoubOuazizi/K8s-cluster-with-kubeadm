...
def generate_inventory():
    ...
    inventory = "[cp_nodes]\ncp_node ansible_host={cp_node_ip} ansible_user=cis-project ansible_ssh_private_key_file=../.ssh/ansible_key\n\n[worker_nodes]".format(cp_node_ip=cp_node_ip)

    for index, worker_node_ip in enumerate(worker_node_ips):
        if worker_node_ip:
            inventory += f"\nworker_node_{index + 1} ansible_host={worker_node_ip.strip().replace(',', '')} ansible_user=cis-project ansible_ssh_private_key_file=../.ssh/ansible_key"
